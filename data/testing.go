...
// IsolateConfigHome 将 FLASHSHELL_CONFIG_HOME 指到临时目录，避免测试写真实 ~/.flashshell。
// 返回隔离后的配置目录路径。
...
func MustIsolateConfigHomeProcess() string {
	dir, err := os.MkdirTemp("", "flashshell-testhome-*")
	if err != nil {
		panic("创建测试配置目录失败: " + err.Error())
	}
	if err := os.Setenv(ConfigHomeEnv, dir); err != nil {
		panic("设置 FLASHSHELL_CONFIG_HOME 失败: " + err.Error())
	}
	ResetAppDataCacheForTest()
	return dir
}
